...
**Reason**: Group assignment proved unreliable end-to-end in the Tauri 2 + WebKitGTK client (see
`group-crud-ipc`/`group-management-ui` removal below for the client-facing symptom); the feature is
being cut entirely rather than kept as a partially-working store-layer capability with no working
client.
**Migration**: None. The `groups` table, `clips.group_id` column, and all group-related
`clip-store` functions (`insert`/`get`/`rename`/`list_all`/`list_children`/`delete`) are dropped by
a new migration. Clips themselves are unaffected; only their group assignment is discarded.
...
**Reason**: Depends entirely on the `groups` table being removed above.
**Migration**: None.
