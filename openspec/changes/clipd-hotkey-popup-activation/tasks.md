...
- [ ] 1.1 Write a failing test in `crates/clipd/src/app.rs`'s test module defining a local
...
- [ ] 1.2 Run `cargo test -p clipd app::` and confirm it fails (no `hotkeys` param, no registration call
      exists yet).
- [ ] 1.3 Add a `hotkeys: Arc<dyn clip_platform::hotkeys::HotkeyBackend>` parameter to `app::run`, and
      ...
      pass it through.
- [ ] 1.4 Write a failing test asserting an invalid/unparseable persisted `hotkey_binding` (or a fake that
...
- [ ] 1.5 Implement the graceful-degradation path (log and continue past registration failure) - minimum
      code to pass.
- [ ] 1.6 Run `cargo test -p clipd` and confirm the full suite is green.

## 2. `clip-ui-tauri`: popup window definition and show/focus on HotkeyPressed

- [ ] 2.1 Add a second, initially-hidden `"popup"` window entry to
...
- [ ] 2.2 In `crates/clip-ui-tauri/src-tauri/src/lib.rs`'s existing event-forwarding closure, add a branch
...
- [ ] 2.3 Run `cargo check -p clip-ui-tauri` and `cargo clippy -p clip-ui-tauri --all-targets -- -D
...
- [ ] 3.1 Write a failing Vitest test in `Popup.test.tsx` asserting that, after initial mount and a
...
- [ ] 3.2 Run `npm test` (inside `crates/clip-ui-tauri`) and confirm it fails (no `daemon-event` listener
      exists in `Popup.tsx` yet).
- [ ] 3.3 Implement a `HotkeyPressed` listener in `src/views/popup/Popup.tsx` that re-runs the existing
...
- [ ] 3.4 Run `npm test` and confirm the full frontend suite (including the existing Popup tests) is green.

## 4. Crate-level and manual verification

- [ ] 4.1 Run `cargo test -p clipd` and `cargo test -p clip-ui-tauri` and confirm every test from sections
      1 and 3 passes.
- [ ] 4.2 Run `cargo check --workspace` and confirm the rest of the workspace still compiles.
- [ ] 4.3 Run `cargo clippy -p clipd -p clip-ui-tauri --all-targets -- -D warnings` and fix any lints
      introduced by this change.
- [ ] 4.4 Run `npm run build` inside `crates/clip-ui-tauri` and confirm the frontend still type-checks and
